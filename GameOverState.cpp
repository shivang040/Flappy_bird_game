...
namespace game
{
    ...
    void GameOverState::Init()
    {
        ...
        _gameOverContainer.setPosition((this->_data->window.getSize().x/2)-(_gameOverContainer.getGlobalBounds().width/2),(this->_data->window.getSize().y/2)-(_gameOverContainer.getGlobalBounds().height/2));
        _gameOverTitle.setPosition((this->_data->window.getSize().x/2)-(_gameOverTitle.getGlobalBounds().width/2),_gameOverContainer.getPosition().y-(_gameOverTitle.getGlobalBounds().height*1.2));
        _retryButton.setPosition((this->_data->window.getSize().x/2)-(_retryButton.getGlobalBounds().width/2),_gameOverContainer.getPosition().y+_gameOverContainer.getGlobalBounds().height+(_retryButton.getGlobalBounds().height*0.2));

        _scoreText.setFont(this->_data->assets.GetFont("Flappy Font"));
        _scoreText.setString(patch::to_string(_score));
        ...
        _highScoreText.setString(patch::to_string(_highScore));
        _highScoreText.setCharacterSize(56);
        _highScoreText.setFillColor(sf::Color::White);
        _highScoreText.setOrigin(_highScoreText.getGlobalBounds().width/2,_highScoreText.getGlobalBounds().height/2);
        _highScoreText.setPosition(sf::Vector2f(this->_data->window.getSize().x/10*7.25,this->_data->window.getSize().y/1.78));
